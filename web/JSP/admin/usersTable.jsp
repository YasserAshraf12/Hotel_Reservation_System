                ...
                <table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    <thead>
                        ...
                        <%
                                ...
                                List<Users> clients = userServices.selectAllUser();
                                for(int i = 0; i < clients.size(); i++)
                                {
                                       out.println("<tr>");
                                       out.println("<td class='userId'>" + clients.get(i).getUserId() + "</td>");
                                       out.println("<td>" + clients.get(i).getUserFirstName() + "</td>");
                                       out.println("<td>" + clients.get(i).getUserLastName() + "</td>");
                                       out.println("<td>" + clients.get(i).getUserEmail() + "</td>");
                                       out.println("<td>" + clients.get(i).getUserAge() + "</td>");
                                       out.println("<td>" + clients.get(i).getUserUserName() + "</td>");
                                       out.println("<td>" + clients.get(i).getCountry().getCountryName() + "</td>");
                                       out.println("<td>" + clients.get(i).getCity().getCityName() + "</td>");
                                       if(clients.get(i).getUserAddress() == null)
                                                out.println("<td>-</td>");
                                        else
                                                out.println("<td>" +  clients.get(i).getUserAddress() + "</td>");
                                       
                                       if(clients.get(i).getUserTelefone() == null)
                                                out.println("<td>-</td>");
                                       else
                                               out.println("<td>" +  clients.get(i).getUserTelefone() + "</td>");
                                       
                                       if(clients.get(i).getUserAccountVerified() == 'Y')
                                                out.println("<td>" + "Verified" + "</td>");
                                       else
                                                out.println("<td>" + "Not Verified" + "</td>");
                                       
                                       out.println("<td><form method='POST'>"
                                               + "<input type= 'submit' name='delete_user'  id='deleteBtn' value='Delete' class='btn btn-primary'>"
                                               + "<input type='hidden' name= 'user' value='" + clients.get(i).getUserId() + "'" 
                                               + "</form></td>");
                                       out.println("</tr>");
                                }
                        %>
